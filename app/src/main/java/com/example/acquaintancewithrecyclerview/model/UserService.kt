package com.example.acquaintancewithrecyclerview.model
...
class UserService {
    ...
    fun findUser(user: User) = users.indexOfFirst { it.id == user.id }

    fun deleteUser(user: User) {
        val index = findUser(user)
        if (index != -1) {
    ...
    fun moveUser(user: User, moveBy: Int) {
        val index = findUser(user)
        val newIndex = index + moveBy
        if (index == -1 || newIndex == -1 || newIndex == users.size) return
        users = ArrayList(users)
        Collections.swap(users, index, newIndex)
        notifyChanges()
    }

    fun fireUser(user: User) {
        val newUser = user.copy(company = "")
        val index = findUser(user)
        if (index != -1) {
            users = ArrayList(users)
            users[index] = newUser
